...
export default function Dashboard() {
  ...
  const canCreateTours = hasPermission("tours:create");
  ...
  ].filter(Boolean) as { label: string; value: string | number; icon: any; trend?: string; trendUp?: boolean; className?: string }[];
  ...
  return (
    <Layout>
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 ${statCards.length >= 4 ? "lg:grid-cols-4" : statCards.length === 3 ? "lg:grid-cols-3" : statCards.length === 2 ? "lg:grid-cols-2" : "lg:grid-cols-1"} gap-6`}>
        {statCards.map((card) => (
          <MetricCard
            key={card.label}
            label={card.label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        ...
        <div className={`bg-primary text-primary-foreground rounded-2xl p-6 shadow-xl shadow-primary/20 relative overflow-hidden ${!canViewFinance ? "lg:col-span-3" : ""}`}>
          ...
          <div className="space-y-3 relative z-10">
            {canCreateTours && (
              <button
                ...
                className="w-full text-left px-4 py-3 rounded-xl bg-white/10 hover:bg-white/20 transition-colors flex items-center justify-between group"
              >
            ...
            {!canCreateTours && !canViewBookings && !hasPermission("settings:edit") && (
              <p className="text-primary-foreground/50 text-sm">No actions available for your role.</p>
            )}
          </div>
        </div>
      </div>
